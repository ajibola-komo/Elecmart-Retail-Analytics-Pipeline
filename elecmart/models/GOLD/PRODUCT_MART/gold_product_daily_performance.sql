...
round((sum(ft.transaction_total) - sum(fs.line_cost)) / sum(nullif(ft.transaction_total, 0)) * 100, 2) as profit_margin_percentage,
...
group by product_id, transaction_date, transaction_date_id
